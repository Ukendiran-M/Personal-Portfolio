...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  useEffect(() => {
    ...
    // Start from the provided items, but reorder them for WebGL so that the
    // default front-facing disc corresponds to the "Frontend" item instead of
    // the geometry's original first visible item (which was "Platform Based").
    const rawItems = items.length ? items : defaultItems;
    let menuItems = rawItems;

    const frontendIdx = rawItems.findIndex(item => item.title === 'Frontend');
    const platformIdx = rawItems.findIndex(item => item.title === 'Platform Based');
    if (frontendIdx >= 0 && platformIdx >= 0 && frontendIdx !== platformIdx) {
      menuItems = [...rawItems];
      const tmp = menuItems[frontendIdx];
      menuItems[frontendIdx] = menuItems[platformIdx];
      menuItems[platformIdx] = tmp;
    }

    // Set an initial active item so the overlay and the visible disc both
    // show "Frontend" on first render.
    const frontendIndex = menuItems.findIndex(item => item.title === 'Frontend');
    const initialIndex = frontendIndex >= 0 ? frontendIndex : 0;
    setActiveItem(menuItems[initialIndex]);
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(canvas, menuItems, handleActiveItem, setIsMoving, sk => sk.run());
    }
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith('http')) {
      ...
      // Internal navigation placeholder
      console.log('Internal route:', activeItem.link);
    }
  };

  const isFrontend = activeItem?.title === 'Frontend';
  const isBackend = activeItem?.title === 'Backend';
  const isPlatformBased = activeItem?.title === 'Platform Based';
  const isDatabase = activeItem?.title === 'Database';
  const isTechStack = activeItem?.title === 'Tech Stack';
  const isBrightLogo = isFrontend || isBackend || isPlatformBased || isDatabase || isTechStack;
  const hideDescription = isBrightLogo;

  return (
    <div className="relative w-full h-full rounded-3xl border-[3px] border-red-500/25 bg-gradient-to-br from-red-950/30 via-black to-purple-950/30 shadow-[0_0_40px_rgba(0,0,0,0.7)] overflow-hidden">
      <canvas
        id="infinite-grid-menu-canvas"
        ref={canvasRef}
        className="cursor-grab w-full h-full relative outline-none active:cursor-grabbing rounded-3xl"
      />

      {!hasInteracted && (
        <div className="pointer-events-none absolute inset-0 flex items-end justify-center pb-6 md:pb-8">
          <div className="flex items-center gap-2 text-[10px] md:text-xs tracking-[0.16em] uppercase text-gray-300/75 animate-pulse">
            <span className="hidden sm:inline-block h-px w-6 bg-gray-500/60" />
            <span className="whitespace-nowrap">Click & drag to rotate</span>
            <span className="hidden sm:inline-block h-px w-6 bg-gray-500/60" />
          </div>
        </div>
      )}
      ...
      {activeItem && (
        <>
          ...
          ${
            isBrightLogo
              ? 'text-black [text-shadow:-1px_-1px_0_#fff,1px_-1px_0_#fff,-1px_1px_0_#fff,1px_1px_0_#fff]'
              : 'text-white drop-shadow-[0_0_18px_rgba(0,0,0,0.8)]'
          }
          ${
            isMoving
        ...
        `}
          ...
          ${
            isMoving
              ...
              : 'bottom-10 md:bottom-12 opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
          }
        `}
          >
